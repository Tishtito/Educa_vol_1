...
<section class="marks-table">
   ...
   if ($_SESSION['marks_out_of7'] === null) {
      ...
      if ($stmt) {
         ...
         if ($row = $result->fetch_assoc()) {
            $_SESSION['marks_out_of7'] = $row['marks_out_of']; // Store in session
         }
         $stmt->close();
      } else {
   ...
   if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['set_marks_out_of7'])) {
      $_SESSION['marks_out_of7'] = intval($_POST['marks_out_of7']); // Store new value in session
      ...
      $stmt = $conn->prepare($sql);
      if ($stmt) {
         $stmt->bind_param("isi", $exam_id, $subject, $_SESSION['marks_out_of7']);
         $stmt->execute();
         $stmt->close();
         ...
         echo "<script>
               setTimeout(function() {
                  swal({
                     title: 'Good job!',
                     text: 'Marks Out Of set to " . $_SESSION['marks_out_of7'] . "',
                     icon: 'success',
                     button: 'OK'
                  }).then(function() {
   ...
   <form method="post">
      <h2>Marks out of: 
         <input type="number" name="marks_out_of7" value="<?php echo htmlspecialchars($_SESSION['marks_out_of7']); ?>" required>
         <button type="submit" name="set_marks_out_of7" class="inline-btn">Set</button>
      </h2>
   </form>
               ...
               <?php if ($result->num_rows > 0): ?>
                  <?php while ($row = $result->fetch_assoc()): ?>
                     <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['SST'] ?? '-'); ?></td>
                        <td>
                           <a class="option-btn" href="../editmarks/editsst.php?student_id=<?php echo htmlspecialchars($row['student_id']); ?>&exam_id=<?php echo htmlspecialchars($exam_id); ?>">
                              Edit
                           </a>
                        </td>
                     </tr>
...
<footer class="footer">
   ...
   &copy; copyright @ 2025 by <span>T&T designer</span> | all rights reserved!
