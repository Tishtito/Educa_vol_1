...
$sql = "
    SELECT 
        ...
        AVG(COALESCE(exam_results.SciTech, 0)) AS 	SciTechMean,
        AVG(COALESCE(exam_results.AgricNutri, 0)) AS AgricNutriMean,
        AVG(COALESCE(exam_results.SST, 0)) AS SSTMean,
        AVG(COALESCE(exam_results.CRE, 0)) AS CreMean
    FROM 
        exam_results 
    INNER JOIN 
...
if ($result->num_rows > 0) {
    ...
    $scitech_mean = round($row['SciTechMean'], 2);
    $agricnutri_mean = round($row['AgricNutriMean'], 2);
    $sst_mean = round($row['SSTMean'], 2);
    $cre_mean = round($row['CreMean'], 2);
} else {
    $english_mean = $kiswahili_mean = $math_mean = $creative_mean = $scitech_mean = $agricnutri_mean = $sst_mean = $scitech_mean = 0;
}
...
$sql = "
    SELECT 
        ...
        (COALESCE(exam_results.English, 0) + COALESCE(exam_results.Math, 0) + COALESCE(exam_results.Kiswahili, 0) + 
         COALESCE(exam_results.Creative, 0) + COALESCE(exam_results.SciTech, 0) + COALESCE(exam_results.AgricNutri, 0) + COALESCE(exam_results.SST, 0) + COALESCE(exam_results.CRE, 0)) AS Total
    FROM 
        students 
    JOIN 
