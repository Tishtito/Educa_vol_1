...
<section class="marks-table">
   ...
   if ($_SESSION['marks_out_of'] === null) {
      ...
      if ($stmt) {
         ...
         if ($row = $result->fetch_assoc()) {
            $_SESSION['marks_out_of'] = $row['marks_out_of']; // Store in session
         }
         $stmt->close();
      } else {
   ...
   if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['set_marks_out_of'])) {
      $_SESSION['marks_out_of'] = intval($_POST['marks_out_of']); // Store new value in session
      ...
      $stmt = $conn->prepare($sql);
      if ($stmt) {
         $stmt->bind_param("isi", $exam_id, $subject, $_SESSION['marks_out_of']);
         $stmt->execute();
         $stmt->close();
         ...
         echo "<script>
               setTimeout(function() {
                  swal({
                     title: 'Good job!',
                     text: 'Marks Out Of set to " . $_SESSION['marks_out_of'] . "',
                     icon: 'success',
                     button: 'OK'
                  }).then(function() {
   ...
   <form method="post">
      <h2>Marks out of: 
         <input type="number" name="marks_out_of" value="<?php echo htmlspecialchars($_SESSION['marks_out_of']); ?>" required>
         <button type="submit" name="set_marks_out_of" class="inline-btn">Set</button>
      </h2>
   </form>
               ...
               <?php if ($result->num_rows > 0): ?>
                  <?php while ($row = $result->fetch_assoc()): ?>
                     <tr>
                        ...
                        <td>
                           <a class="option-btn" href="../editmarks/editkiswahili.php?student_id=<?php echo htmlspecialchars($row['student_id']); ?>&exam_id=<?php echo htmlspecialchars($exam_id); ?>">
                              Edit
                           </a>
                        </td>
                     </tr>
