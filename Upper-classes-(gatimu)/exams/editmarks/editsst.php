...
<body>
    <?php
        ...
        if (!isset($_SESSION["exam_id"], $_SESSION["marks_out_of7"])) {
            echo "<script>
                swal({
                    title: 'Caution!',
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (!is_numeric($marks) || $marks < 0 || $marks > $marks_out_of) {
                ...
                if ($exists) {
                    ...
                    $sql = "INSERT INTO exam_results (exam_id, student_class_id, SST)
                            VALUES (?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                ...
                if ($stmt->execute()) {
                    echo "<script>
                        swal({
                            ...
                            icon: 'success'
                        }).then(() => window.location.href = '../subjects/sst.php');
                    </script>";
                    exit;
                }
