...
<body>
    <?php
        ...
        if (!isset($_SESSION["exam_id"], $_SESSION["marks_out_of"])) {
            echo "<script>
                swal({
                    title: 'Caution!',
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $sql = "
                SELECT s.name, er.Kiswahili, sc.class
            ...
            if (!is_numeric($marks) || $marks < 0 || $marks > $marks_out_of) {
                ...
                if ($exists) {
                    ...
                    $sql = "INSERT INTO exam_results (exam_id, student_class_id, Kiswahili)
                            VALUES (?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                ...
                if ($stmt->execute()) {
                    echo "<script>
                        swal({
                            ...
                            icon: 'success'
                        }).then(() => window.location.href = '../subjects/kiswahili.php');
                    </script>";
                    exit;
                }
        ...
        <?php endif; ?>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-3 d-grid">
                    <a class="btn btn-outline-primary mb-2" href="../subjects/kiswahili.php" role="button">Cancel</a>
                </div>
            </div>
        </form>
    </div>
