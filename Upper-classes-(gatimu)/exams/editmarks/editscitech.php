...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (!isset($_GET["student_id"])) {
        header("location: ../subjects/scitech.php");
        exit;
    }
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE exam_results SET SciTech = ? WHERE student_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("dii", $percentage_marks, $id, $exam_id);
        } else {
            // Insert a new record
            $sql = "INSERT INTO exam_results (exam_id, student_id, SciTech) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
