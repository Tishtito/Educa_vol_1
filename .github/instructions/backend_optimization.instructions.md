...
## 3. Cache Rules (MANDATORY)

- All read-heavy endpoints must use caching:

```php

$this->cache->remember('cache:key', 60, function () {
    return $this->db->select(...);
});

```
- After ANY mutation (insert/update/delete): Controllers MUST invalidate related cache keys.
Example:
```php
$this->cache->forget('users:teachers');
```