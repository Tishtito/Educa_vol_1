    ...
    <?php
        ...
        $sql = "
            SELECT 
                ...
                COALESCE(exam_results.Agriculture, 0) AS Agriculture,
                COALESCE(exam_results.SST, 0) AS SST,
                COALESCE(exam_results.Technical, 0) AS Technical,
                COALESCE(exam_results.Science, 0) AS Science
            FROM 
                students
            LEFT JOIN 
                exam_results 
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($student = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?php echo calculateGrade($student['Agriculture']); ?></td>
                            <td><?php echo calculateGrade($student['SST']); ?></td>
                            <td><?php echo calculateGrade($student['Technical']); ?></td>
                            <td><?php echo calculateGrade($student['Science']); ?></td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr>
