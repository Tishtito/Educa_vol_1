...
$sql = "
    ...
    JOIN (
        SELECT student_id, 
            ...
            COALESCE(SST, 0) + COALESCE(Agriculture, 0) + COALESCE(Religious, 0)) AS total_marks
        FROM exam_results
        WHERE exam_id = ?
    ) total_marks ON exam_results.student_id = total_marks.student_id
    SET exam_results.total_marks = total_marks.total_marks";
...
$sql = "
    ...
    ) ranked ON exam_results.student_id = ranked.student_id
    SET exam_results.position = ranked.position";
...
$sql = "
    SELECT 
        ...
        er.Technical, (SELECT ab FROM point_boundaries WHERE er.Technical BETWEEN min_marks AND max_marks LIMIT 1) AS PL_Technical,
        er.Agriculture, (SELECT ab FROM point_boundaries WHERE er.Agriculture BETWEEN min_marks AND max_marks LIMIT 1) AS PL_Agriculture,
        er.SST, (SELECT ab FROM point_boundaries WHERE er.SST BETWEEN min_marks AND max_marks LIMIT 1) AS PL_SST,
        er.Science, (SELECT ab FROM point_boundaries WHERE er.Science BETWEEN min_marks AND max_marks LIMIT 1) AS PL_Science,
        er.Religious, (SELECT ab FROM point_boundaries WHERE er.Religious BETWEEN min_marks AND max_marks LIMIT 1) AS PL_Religious,
        er.total_marks
    FROM 
        students s
    LEFT JOIN 
    ...
    <table>
        ...
        <tbody>
            ...
            foreach ($students as $row) {
                ...
                echo "<td>" . htmlspecialchars($row['PL_Total'] ?? '-') . "</td>"; // Add this line
