(async function () {
    ...
    try {
        ...
        const rows = Object.keys(subjects || {}).map((key, index) => {
            ...
            const endVal = end_results ? end_results[key] : null;
            ...
            return `<tr>
                ...
                <td class="total">${getPerformanceLevel(endVal, levels)}</td>
            </tr>`;
        });
        body.innerHTML = rows.join("");
        document.getElementById('total-mid').textContent = totalMid;
        document.getElementById('total-end').textContent = totalEnd;
    } catch (error) {
