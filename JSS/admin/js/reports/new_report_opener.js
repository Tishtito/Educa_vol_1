(async function () {
    ...
    try {
        ...
        document.getElementById('exam-year').textContent = exam.exam_year ? ` ${exam.exam_year}` : '';
        document.getElementById('exam-type-label').textContent = exam.exam_type || 'Exam';
        document.getElementById('exam-header').textContent = `${exam.exam_type || 'Exam'} out of 100`;
        ...
        const bodyRows = subjectKeys.map((key) => {
            ...
            return `<tr>
                <td>${subjectName}</td>
                <td>${score ?? '-'}</td>
                <td>${performance.ab}</td>
                <td>-</td>
                <td>-</td>
                <td>${score !== null && score !== undefined ? `${score} %` : '-'}</td>
                <td>${performance.ab}</td>
                <td>${performance.pl}</td>
            </tr>`;
        });
