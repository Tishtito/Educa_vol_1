...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE exam_results SET Agriculture = ? WHERE student_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("dii", $percentage_marks, $id, $exam_id);
        } else {
            // Insert a new record
            $sql = "INSERT INTO exam_results (exam_id, student_id, Agriculture) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
        ...
        setTimeout(function() {
            swal({
                ...
                window.location.href = '../subjects/agriculture.php?subject_id=" . urlencode($_SESSION['subject_id']) . "&class_id=" . urlencode($_SESSION['class_id']) . "&class_name=" . urlencode($_SESSION['class_name']) . "';
            });
        }, 100);
    </script>";
        exit;
