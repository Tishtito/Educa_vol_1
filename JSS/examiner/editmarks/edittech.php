...
if (!isset($_SESSION["exam_id"]) || !isset($_SESSION["marks_out_of8"])) {
...
$marks_out_of = $_SESSION['marks_out_of8']; // Retrieve marks out of from session
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (!isset($_GET["student_id"])) {
        header("Location: ../subjects/Technical.php");
        exit;
    }

    $id = $_GET["student_id"];

    $sql = "SELECT s.name AS Name, er.Technical, sc.student_class_id
            FROM students s
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            // Update
            $sql = "UPDATE exam_results SET Technical = ?, student_id = ? WHERE student_class_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("diii", $percentage_marks, $id, $student_class_id, $exam_id);
        } else {
            // Insert
            $sql = "INSERT INTO exam_results (exam_id, student_id, student_class_id, Technical) VALUES (?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
        ...
        echo "<script>
            setTimeout(function() {
                swal({
                    ...
                    window.location.href = '../subjects/pre-technical.php?subject_id=" . urlencode($_SESSION['subject_id']) . "&class_id=" . urlencode($_SESSION['class_id']) . "&class_name=" . urlencode($_SESSION['class_name']) . "';
                });
            }, 100);
        </script>";
        exit;
