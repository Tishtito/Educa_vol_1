...
<body>
<?php
if (!isset($_SESSION["exam_id"]) || !isset($_SESSION["marks_out_of6"])) {
...
$marks_out_of = $_SESSION['marks_out_of6']; // Retrieve marks out of from session
$id = "";
$name = "";
$marks = "";
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE exam_results SET English = ? WHERE student_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("dii", $percentage_marks, $id, $exam_id);
        } else {
            // Insert a new record
            $sql = "INSERT INTO exam_results (exam_id, student_id, English) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
