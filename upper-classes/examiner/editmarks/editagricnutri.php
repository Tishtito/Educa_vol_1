...
if (!isset($_SESSION["exam_id"]) || !isset($_SESSION["marks_out_of4"]) || !isset($_SESSION['class_id'])) {
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sql = "SELECT s.name AS Name, er.AgricNutri, sc.student_class_id
            FROM students s
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            // Update
            $sql = "UPDATE exam_results SET AgricNutri = ?, student_id = ? WHERE student_class_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("diii", $percentage_marks, $id, $student_class_id, $exam_id);
        } else {
            // Insert
            $sql = "INSERT INTO exam_results (exam_id, student_id, student_class_id, AgricNutri) VALUES (?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
