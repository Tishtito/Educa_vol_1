...
if (!isset($_SESSION["exam_id"]) || !isset($_SESSION["marks_out_of4"])) {
    ...
    echo "<script>
            setTimeout(function() {
                  swal({
                     ...
                     window.location.href = '../home.php'; // Reload page to reflect changes
                  });
            }, 100);
         </script>";
}
...
$marks_out_of = $_SESSION['marks_out_of4']; // Retrieve marks out of from session
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (!isset($_GET["student_id"])) {
        header("location: ../subjects/agriculture and nutrition.php");
        exit;
    }
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE exam_results SET AgricNutri = ? WHERE student_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("dii", $percentage_marks, $id, $exam_id);
        } else {
            // Insert a new record
            $sql = "INSERT INTO exam_results (exam_id, student_id, AgricNutri) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
        ...
        echo "<script>
            setTimeout(function() {
                swal({
                    ...
                    window.location.href = '../subjects/agriculture and nutrition.php?subject_id=" . urlencode($_SESSION['subject_id']) . "&class_id=" . urlencode($_SESSION['class_id']) . "&class_name=" . urlencode($_SESSION['class_name']) . "';
                });
            }, 100);
        </script>";
        exit;
