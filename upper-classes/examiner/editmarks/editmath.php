...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sql = "SELECT students.name AS Name, exam_results.Math 
            FROM students 
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE exam_results SET Math = ? WHERE student_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("dii", $percentage_marks, $id, $exam_id);
        } else {
            // Insert a new record
            $sql = "INSERT INTO exam_results (exam_id, student_id, Math) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
