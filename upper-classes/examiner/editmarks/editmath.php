...
if (!isset($_SESSION["exam_id"]) || !isset($_SESSION["marks_out_of3"]) || !isset($_SESSION['class_id'])) {
...
$marks_out_of = $_SESSION['marks_out_of3'];
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sql = "SELECT s.name AS Name, er.Math, sc.student_class_id
            FROM students s
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            // Update
            $sql = "UPDATE exam_results SET Math = ?, student_id = ? WHERE student_class_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql); 
            $stmt->bind_param("diii", $percentage_marks, $id, $student_class_id, $exam_id);
        } else {
            // Insert
            $sql = "INSERT INTO exam_results (exam_id, student_id, student_class_id, Math) VALUES (?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
        ...
        echo "<script>
            setTimeout(function() {
                swal({
                    ...
                    window.location.href = '../subjects/math.php?subject_id=" . urlencode($_SESSION['subject_id']) . "&class_id=" . urlencode($_SESSION['class_id']) . "&class_name=" . urlencode($_SESSION['class_name']) . "';
                });
            }, 100);
        </script>";
        exit;
