...
<section class="marks-table">
   ...
   <?php
      ...
      if (isset($_POST['set_marks_out_of2'])) {
         $_SESSION['marks_out_of2'] = intval($_POST['marks_out_of2']); // Store the new value
         ...
         echo "<script>
            setTimeout(function() {
                  swal({
                     title: 'Good job!',
                     text: 'Marks Out Of set to " . $_SESSION['marks_out_of2'] . "',
                     icon: 'success',
                     button: 'OK'
                  }).then(function() {
   ...
   <form method="post">
      <h2>Marks out of: 
         <input type="number" name="marks_out_of2" value="<?php echo $_SESSION['marks_out_of2']; ?>" required>
         <button type="submit" name="set_marks_out_of2" class="inline-btn">Set</button>
      </h2>
   </form>
   <?php
