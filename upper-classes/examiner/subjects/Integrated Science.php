...
<head>
    ...
    <title>Integrated Science</title>
...
<div class="side-bar">
   ...
   <nav class="navbar">
      ...
      <a href="https://upperclaIntegrated Scienceeacher.nguresch.xyz/exam.php"><i class="fas fa-chalkboard-teacher"></i><span>Class-Teacher Portal</span></a>
   </nav>
...
<section class="marks-table">
   <h1 class="heading">Intergrated Science</h1>
   <?php
      // Initialize session value if not set
      if (!isset($_SESSION['marks_out_8'])) {
         $_SESSION['marks_out_8'] = null;
      }

      if (isset($_POST['set_marks_out_8'])) {
         $_SESSION['marks_out_8'] = intval($_POST['marks_out_8']); // Store the new value
         ...
         echo "<script>
            setTimeout(function() {
                  swal({
                     title: 'Good job!',
                     text: 'Marks Out Of set to " . $_SESSION['marks_out_8'] . "',
                     icon: 'success',
                     button: 'OK'
                  }).then(function() {
   ...
   <form method="post">
      <h2>Marks out of: 
         <input type="number" name="marks_out_8" value="<?php echo $_SESSION['marks_out_8']; ?>" required>
         <button type="submit" name="set_marks_out_8" class="inline-btn">Set</button>
      </h2>
   </form>
