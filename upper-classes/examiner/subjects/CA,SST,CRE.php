...
<div class="side-bar">
   ...
   <nav class="navbar">
      ...
      <a href="https://upperclaCA_SST_CREeacher.nguresch.xyz/exam.php"><i class="fas fa-chalkboard-teacher"></i><span>Class-Teacher Portal</span></a>
   </nav>
...
<section class="marks-table">
   <h1 class="heading">CA_SST_CRE</h1>
   <?php
      // Initialize session value if not set
      if (!isset($_SESSION['marks_out_9'])) {
         $_SESSION['marks_out_9'] = null;
      }

      if (isset($_POST['set_marks_out_9'])) {
         $_SESSION['marks_out_9'] = intval($_POST['marks_out_9']); // Store the new value
         ...
         echo "<script>
            setTimeout(function() {
                  swal({
                     title: 'Good job!',
                     text: 'Marks Out Of set to " . $_SESSION['marks_out_9'] . "',
                     icon: 'success',
                     button: 'OK'
                  }).then(function() {
   ...
   <form method="post">
      <h2>Marks out of: 
         <input type="number" name="marks_out_9" value="<?php echo $_SESSION['marks_out_9']; ?>" required>
         <button type="submit" name="set_marks_out_9" class="inline-btn">Set</button>
      </h2>
   </form>
               ...
               <?php if ($result->num_rows > 0): ?>
                  <?php while ($row = $result->fetch_assoc()): ?>
                     <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['CA_SST_CRE'] ?? '-'); ?></td>
                        <td>
                           <a class="option-btn" href="../editmarks/editca_sst_cre.php.php?student_id=<?php echo htmlspecialchars($row['student_id']); ?>&exam_id=<?php echo htmlspecialchars($exam_id); ?>">
                              Edit
                           </a>
                        </td>
                     </tr>
