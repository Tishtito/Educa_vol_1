...
try {
    ...
    if ($target_class_id === "FINISHED") {
        ...
        while ($student = $result->fetch_assoc()) {
            ...
            // ✅ Update both status and class
            $update = $conn->prepare("UPDATE students SET status = 'Finished', class = 'Completed' WHERE student_id = ?");
    ...
    while ($student = $result->fetch_assoc()) {
        ...
        $update = $conn->prepare("UPDATE students SET class = ?, status = 'Active' WHERE student_id = ?");
        $update->bind_param("si", $target_class_name, $student_id);
        $update->execute();
