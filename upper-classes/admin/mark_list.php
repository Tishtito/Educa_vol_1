...
// Get previous means
$prev_mean_scores = array_fill_keys($subjects, null);
$prev_mean_total_marks = null;
...
if ($prev_exam_id) {
    ...
    if ($row = $result->fetch_assoc()) {
        foreach ($subjects as $subject) {
            $val = $row[$subject] ?? null;
            $prev_mean_scores[$subject] = is_numeric($val) ? floatval($val) : null;
        }
        $prev_mean_total_marks = is_numeric($row['total_mean']) ? floatval($row['total_mean']) : null;
    }
}
...
$deviation_scores = [];
foreach ($subjects as $subject) {
    $prev = $prev_mean_scores[$subject] ?? null;
    $curr = $mean_scores[$subject] ?? null;
    
    if (is_numeric($prev) && is_numeric($curr)) {
        $deviation_scores[$subject] = round($curr - $prev, 2);
    } else {
        $deviation_scores[$subject] = '-';
    }
}

// Similarly fix the total mean deviation
if ($prev_mean_total_marks !== null && $mean_total_marks !== null && 
    is_numeric($prev_mean_total_marks) && is_numeric($mean_total_marks)) {
