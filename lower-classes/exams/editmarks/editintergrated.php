    ...
    <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (!isset($_GET["student_class_id"])) {
                header("Location: ../subjects/integrated.php");
                exit;
            }
            ...
            $sql = "
                SELECT s.name, er.Integrated, sc.class
            ...
            if (!is_numeric($marks) || $marks < 0 || $marks > $marks_out_of) {
                ...
                if ($exists) {
                    ...
                    $sql = "INSERT INTO exam_results (exam_id, student_class_id, Integrated)
                            VALUES (?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                ...
                if ($stmt->execute()) {
                    echo "<script>
                        swal({
                            ...
                            icon: 'success'
                        }).then(() => window.location.href = '../subjects/integrated.php');
                    </script>";
                    exit;
                }
        ...
        <?php endif; ?>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-3 d-grid">
                    <a class="btn btn-outline-primary mb-2" href="../subjects/integrated.php" role="button">Cancel</a>
                </div>
            </div>
        </form>
    </div>
