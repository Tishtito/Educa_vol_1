...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE exam_results SET Religious = ? WHERE student_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("dii", $percentage_marks, $id, $exam_id);
        } else {
            // Insert a new record
            $sql = "INSERT INTO exam_results (exam_id, student_id, Religious) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
        ...
        echo "<script>
            setTimeout(function() {
                swal({
                    ...
                    window.location.href = '../subjects/religious.php?. exam_id=" . urlencode($_SESSION['exam_id']) . "';
                });
            }, 100);
        </script>";
        exit;
        ...
        <?php endif; ?>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-3 d-grid">
                    <a class="btn btn-outline-primary mb-2" href="../subjects/religious.php" role="button">Cancel</a>
                </div>
            </div>
        </form>
    </div>
