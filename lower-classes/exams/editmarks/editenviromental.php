...
if (!isset($_SESSION["exam_id"]) || !isset($_SESSION["marks_out_of3"])) {
...
$marks_out_of = $_SESSION['marks_out_of3']; // Retrieve marks out of from session
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (!isset($_GET["student_id"])) {
        header("location: ../subjects/enviromental.php");
        exit;
    }
    ...
    do {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE exam_results SET Enviromental = ? WHERE student_id = ? AND exam_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("dii", $percentage_marks, $id, $exam_id);
        } else {
            // Insert a new record
            $sql = "INSERT INTO exam_results (exam_id, student_id, Enviromental) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
        ...
        echo "<script>
            setTimeout(function() {
                swal({
                    ...
                    window.location.href = '../subjects/enviromental.php?. exam_id=" . urlencode($_SESSION['exam_id']) . "';
                });
            }, 100);
        </script>";
        exit;
        ...
        <?php endif; ?>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-3 d-grid">
                    <a class="btn btn-outline-primary mb-2" href="../subjects/enviromental.php" role="button">Cancel</a>
                </div>
            </div>
        </form>
    </div>
