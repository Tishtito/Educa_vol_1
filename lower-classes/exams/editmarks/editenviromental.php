...
<body>
    <?php
        ...
        if (!isset($_SESSION["exam_id"], $_SESSION["marks_out_of3"])) {
            echo "<script>
                swal({
                    title: 'Caution!',
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $sql = "
                SELECT s.name, er.Enviromental, sc.class
            ...
            if (!is_numeric($marks) || $marks < 0 || $marks > $marks_out_of) {
                ...
                if ($exists) {
                    ...
                    $sql = "INSERT INTO exam_results (exam_id, student_class_id, Enviromental)
                            VALUES (?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                ...
                if ($stmt->execute()) {
                    echo "<script>
                        swal({
                            ...
                            icon: 'success'
                        }).then(() => window.location.href = '../subjects/enviromental.php');
                    </script>";
                    exit;
                }
