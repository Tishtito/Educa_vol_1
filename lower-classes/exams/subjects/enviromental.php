...
<section class="marks-table">
   <h1 class="heading">Enviromental</h1>
   <?php
   ...
   if (!isset($_SESSION['marks_out_of3'])) {
      $_SESSION['marks_out_of3'] = null;
   }

   // Fetch existing "Marks Out Of" from the database if it's not in session
   if ($_SESSION['marks_out_of3'] === null) {
      ...
      if ($stmt) {
         ...
         if ($row = $result->fetch_assoc()) {
            $_SESSION['marks_out_of3'] = $row['marks_out_of']; // Store in session
         }
         $stmt->close();
      } else {
   ...
   if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['set_marks_out_of3'])) {
      $_SESSION['marks_out_of3'] = intval($_POST['marks_out_of3']); // Store new value in session
      ...
      $stmt = $conn->prepare($sql);
      if ($stmt) {
         $stmt->bind_param("isi", $exam_id, $subject, $_SESSION['marks_out_of3']);
         $stmt->execute();
         $stmt->close();
         ...
         echo "<script>
               setTimeout(function() {
                  swal({
                     title: 'Good job!',
                     text: 'Marks Out Of set to " . $_SESSION['marks_out_of3'] . "',
                     icon: 'success',
                     button: 'OK'
                  }).then(function() {
   ...
   <form method="post">
      <h2>Marks out of: 
         <input type="number" name="marks_out_of3" value="<?php echo htmlspecialchars($_SESSION['marks_out_of3']); ?>" required>
         <button type="submit" name="set_marks_out_of3" class="inline-btn">Set</button>
      </h2>
   </form>
   <?php
               ...
               <?php if ($result->num_rows > 0): ?>
                  <?php while ($row = $result->fetch_assoc()): ?>
                     <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['Enviromental'] ?? '-'); ?></td>
                        <td>
                           <a class="option-btn" href="../editmarks/editenviromental.php?student_id=<?php echo htmlspecialchars($row['student_id']); ?>&exam_id=<?php echo htmlspecialchars($exam_id); ?>">
                              Edit
                           </a>
                        </td>
                     </tr>
