...
<section class="marks-table">
   <h1 class="heading">Integrated Learing Area</h1>
   <?php
   ...
   if ($_SESSION['marks_out_of6'] === null) {
      ...
      if ($stmt) {
         ...
         if ($row = $result->fetch_assoc()) {
            $_SESSION['marks_out_of6'] = $row['marks_out_of']; // Store in session
         }
         $stmt->close();
      } else {
   ...
   if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['set_marks_out_of6'])) {
      $_SESSION['marks_out_of6'] = intval($_POST['marks_out_of6']); // Store new value in session
               ...
               <?php if ($result->num_rows > 0): ?>
                  <?php while ($row = $result->fetch_assoc()): ?>
                     <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['Integrated'] ?? '-'); ?></td>
                        <td>
                           <a class="option-btn" href="../editmarks/editintergrated.php?student_id=<?php echo htmlspecialchars($row['student_id']); ?>&exam_id=<?php echo htmlspecialchars($exam_id); ?>">
                              Edit
                           </a>
                        </td>
                     </tr>
