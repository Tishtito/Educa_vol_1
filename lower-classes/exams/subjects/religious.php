...
<section class="marks-table">
   <h1 class="heading">Religious Activites</h1>
   <?php
   ...
   if (!isset($_SESSION['marks_out_of5'])) {
      $_SESSION['marks_out_of5'] = null;
   }

   // Fetch existing "Marks Out Of" from the database if it's not in session
   if ($_SESSION['marks_out_of5'] === null) {
      ...
      if ($stmt) {
         ...
         if ($row = $result->fetch_assoc()) {
            $_SESSION['marks_out_of5'] = $row['marks_out_of']; // Store in session
         }
         $stmt->close();
      } else {
   ...
   if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['set_marks_out_of5'])) {
      $_SESSION['marks_out_of5'] = intval($_POST['marks_out_of5']); // Store new value in session
      ...
      $stmt = $conn->prepare($sql);
      if ($stmt) {
         $stmt->bind_param("isi", $exam_id, $subject, $_SESSION['marks_out_of5']);
         $stmt->execute();
         $stmt->close();
         ...
         echo "<script>
               setTimeout(function() {
                  swal({
                     title: 'Good job!',
                     text: 'Marks Out Of set to " . $_SESSION['marks_out_of5'] . "',
                     icon: 'success',
                     button: 'OK'
                  }).then(function() {
   ...
   <form method="post">
      <h2>Marks out of: 
         <input type="number" name="marks_out_of5" value="<?php echo htmlspecialchars($_SESSION['marks_out_of5']); ?>" required>
         <button type="submit" name="set_marks_out_of5" class="inline-btn">Set</button>
      </h2>
   </form>
               ...
               <?php if ($result->num_rows > 0): ?>
                  <?php while ($row = $result->fetch_assoc()): ?>
                     <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['Religious'] ?? '-'); ?></td>
                        <td>
                           <a class="option-btn" href="../editmarks/editreligious.php?student_id=<?php echo htmlspecialchars($row['student_id']); ?>&exam_id=<?php echo htmlspecialchars($exam_id); ?>">
                              Edit
                           </a>
                        </td>
                     </tr>
