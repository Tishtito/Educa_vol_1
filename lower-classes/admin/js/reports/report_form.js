/**
 * Combined Performance Report Module
 * Handles loading and displaying combined mid-term and end-term performance reports
 */

const CombinedPerformanceReport = (() => {
   const BASE_URL = "../../backend/public/index.php";

   // Cache DOM elements
   let studentNameEl, studentClassEl, tutorNameEl;
   let termLabelEl, examYearEl;
   let subjectsBody, totalMidEl, totalEndEl;
   ...
   function cacheDOMElements() {
      ...
      printButton = document.getElementById("printInvoice");
   }
   ...
   function showLoadingState() {
      if (subjectsBody) {
         subjectsBody.innerHTML = '<tr><td colspan="6">Loading report data...</td></tr>';
      }
   }
   ...
   function showErrorState(message) {
      if (subjectsBody) {
         subjectsBody.innerHTML = `<tr><td colspan="6" style="color: #e74c3c;">${escapeHtml(message)}</td></tr>`;
      }
   }
   ...
   function renderReport(data) {
      ...
      termLabelEl.textContent = escapeHtml(term || '-');
      examYearEl.textContent = escapeHtml(String(exam_year || '-'));

      // Render subjects table
      renderSubjectsTable(subjects, mid_results, end_results, levels);
   }

   /**
    * Render subjects table with batch rendering
    */
   function renderSubjectsTable(subjects, midResults, endResults, levels) {
      const subjectKeys = Object.keys(subjects || {});

      if (subjectKeys.length === 0) {
         subjectsBody.innerHTML = '<tr><td colspan="6">No subject data available</td></tr>';
         return;
      }
      ...
      const rows = subjectKeys.map((key, index) => {
         ...
         const midVal = midResults ? midResults[key] : null;
         const endVal = endResults ? endResults[key] : null;

         // Track totals
         if (midVal !== null && midVal !== undefined) totalMid += Number(midVal);
         if (endVal !== null && endVal !== undefined) totalEnd += Number(endVal);

         return createSubjectRow(index, subjectName, midVal, endVal, levels);
      }).join('');

      subjectsBody.innerHTML = rows;

      // Update totals
      totalMidEl.textContent = escapeHtml(String(totalMid));
      totalEndEl.textContent = escapeHtml(String(totalEnd));
   }

   /**
    * Create individual subject row HTML
    */
   function createSubjectRow(index, subjectName, midVal, endVal, levels) {
      const midPerf = getPerformanceLevel(midVal, levels);
      const endPerf = getPerformanceLevel(endVal, levels);

      const displayMid = midVal ?? '-';
      const displayEnd = endVal ?? '-';

      return `
         <tr>
            <td class="no">${index + 1}</td>
            <td class="text-left"><h3>${escapeHtml(subjectName)}</h3></td>
            <td class="unit">${escapeHtml(String(displayMid))}</td>
            <td class="total">${escapeHtml(midPerf)}</td>
            <td class="unit">${escapeHtml(String(displayEnd))}</td>
            <td class="total">${escapeHtml(endPerf)}</td>
         </tr>
      `;
   }
